...
| 用途 | 可否 |
|---|---|
| 充作**作者所接受之医学命题**（正证或反例） | ⛔⛔ **不可** ——⭐ 这是原禁令之真正所指 |
| 作 **text-critical evidence**（该处文本有疑） | ⭐ **可** |
| ⭐ 作 **研究材料**：作者之**校勘行为**、**拒释范围**、**其理论处理方式** | ⭐⭐ **可，且应做** |

⚠ ⭐ **实例**：§134「注家有说是王叔和搞的，**我认为也是**」——
· ⛔ 不可据此证明该四句**实际出自王叔和**（作者只是**赞同一个意见**）；
· ⭐ **可**据此研究：作者**接受何种校勘依据**、**对哪类文字拒释**、**拒释后如何处置该条**。
⚠ 另：作者对该段之**解说**（「又主热，又主虚」）仍是作者之言，
　只是它是**对一段可疑文本的解说** ⇒ 作 `contextual_exception` 尚可，作 `generic_support` 不可。
⛔ **原文真实性另核**，⛔ 不因「作者赞同」而定。
...
## ⛔⛔ 写「不能确认」之前（126AA 上级裁决）

> 上级 126AA 原文：「**有可行动作就执行；确实缺原始载体或可区分观察时，明确缺什么。
> ⛔ 不能为了获准写『未知』而编造检索任务。**」

**背景**：本工程之免责条款有一个可观察的副作用——
**每写一次「本工程无手段」「材料不足」，下一批就没有人再碰那一处。**
（126U 页码残迹｜126T 小青龙组合授权｜§62 之矛盾，皆躺了数批。）
