...
【已知失效模式】(㉓批·复盘视角㉕ 强制格式)
  ① **只查行号，不查内容归属**——早期版本仅校验"锚是否指向◆行"，106个锚全部通过
     而实际归属错乱。故现版必查三项(锚落点/对举方成员/条目归属)；若日后有人删去
     后两项，本工具立刻退化为无效。
  ② `--fix` 按 ◆段名◆ 重算行号：**段名重复或段名被改写时会挂到错的段上**。
     附录X 段名唯一性是本工具的隐含前提，未被校验。
  ③ 对**尚未回挂到条目**的边完全沉默——它只校验已有的锚，不知道缺了哪些边
     (那是 p8_metrics.py 的事)。W-1.14 那类"表里有而条目无"的错，本工具查不出。
【弃件条件】
  条目名解析失败(【…】头不匹配)时**跳过并计入不在库**，不猜测归属(协议4)。
"""
import re, sys, os
