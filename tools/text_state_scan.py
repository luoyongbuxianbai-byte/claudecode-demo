...
⛔⛔ **本工具之限度（须与结果同读）**
  ① 它只查**已知形态**（见账本 `gap_marker_forms`）＋若干附加标记。
     ⛔ **下一种形态它查不出来** —— 126AI 之第六种、126AJ 之第七种、
     **126AK 之第八种（单点时间码 `（56：00）`）都是【人读时撞见的】**，⛔ 无一由机器找出。
     ⇒ ⭐⭐ **三次皆如此** ⇒ **此工具⛔ 不能取代抽查复读**，且⛔ **每次补完模式后
       仍须假定还有第九种**。
...
PATTERNS = [
    ...
    ("标记·时间码·区间", r"[（(]\s*\d{1,3}\s*[：:]\s*\d{1,2}\s*[—\-－~～]\s*\d{1,3}\s*[：:]\s*\d{1,2}", 26),
    # ⭐ **第八种形态（126AK）**：**单点**时间码 `（56：00）` ——
    #   ⛔ 126AJ 之模式只认「区间」（须有破折号），故 §161 之此处**漏检**，
    #   ⭐ **是人读撞见的** ⇒ **限度①之第三次实证**。
    ("标记·时间码·单点", r"[（(]\s*\d{1,3}\s*[：:]\s*\d{1,2}\s*[）)]", 12),
